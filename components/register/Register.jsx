...
export default function Register({ navigation }) {
  ...
  const { email, password, firstName, lastName, confirmPassword } = login;
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (e, gesture) => {
        if (gesture.dx > screenWidth * 0.25) {
          // Geçiş yapılacak mesafe eşik değeri (0.5 olarak belirlendi)
          navigation.navigate("Home"); // Ana sayfaya geçiş yap
        } else {
          Animated.spring(pan, {
            toValue: { x: 0, y: 0 },
            useNativeDriver: false,
  ...
  return (
    <View style={styles.backgroundContainer}>
      <Animated.View
        className="flex-1 flex items-center justify-center bg-slate-800 pt-2"
        style={[{ transform: [{ translateX: pan.x }] }]}
        {...panResponder.panHandlers}
      >
        <SafeAreaView className="flex h-screen flex-col items-center justify-start bg-slate-800 pt-10">
          {hideLogo ? (
      ...
      c43.4,43.4,113.8,43.4,157.1,0l78.6-78.6C433.1,578.5,433.1,508.2,389.7,464.8z"
          ...
          <View className="bg-slate-800 w-screen flex justify-center items-center ">
            <Text className="text-white text-2xl font-bold mt-5">Kaydol</Text>
            <Text className="text-white text-md font-medium my-5">
              Kayıt Olmak İçin Aşağıdaki Bilgileri Doldurun.
            </Text>

            <TextInput
              value={firstName}
              onFocus={toggleTextInput}
              onBlur={toggleTextInput}
              onChangeText={(text) => registerhandler("firstName", text)}
              placeholder="Ad"
            ...
            <TextInput
              ...
              onFocus={toggleTextInput}
              onBlur={toggleTextInput}
              onChangeText={(text) => registerhandler("lastName", text)}
              placeholder="Soyad"
              className="border-2 border-slate-600 bg-white p-2 w-64 rounded-xl mb-2 text-gray-600 font-bold"
            />
            <TextInput
              ...
              onFocus={toggleTextInput}
              onBlur={toggleTextInput}
              onChangeText={(text) => registerhandler("email", text)}
              placeholder="Eposta"
              className="border-2 border-slate-600 bg-white p-2 w-64 rounded-xl mb-2 text-gray-600 font-bold"
            />
            <TextInput
              ...
              onFocus={toggleTextInput}
              onBlur={toggleTextInput}
              onChangeText={(text) => registerhandler("password", text)}
              placeholder="Şifre"
              className="border-2 border-slate-600 bg-white p-2 w-64 rounded-xl mb-2 text-gray-600 font-bold"
            ...
            <TextInput
              ...
              onFocus={toggleTextInput}
              onBlur={toggleTextInput}
              onChangeText={(text) => registerhandler("confirmPassword", text)}
            ...
            <View className="w-full mt-2 flex flex-row justify-center items-center">
              <TouchableHighlight className="flex flex-row justify-center items-center w-full">
                <Text
                  onPress={() => navigation.navigate("Home", { home: "home" })}
                  className="text-white text-md bg-orange-600 py-2 w-2/3 px-6 rounded-xl text-center"
                >
                  Ana Sayfa
                </Text>
              </TouchableHighlight>
