...
const DashboardNavbar = () => {
  ...
  const [openProductsGroup, setOpenProductsGroup] = useState(false);

  const screenWidth = Dimensions.get("window").width;
  const panX = useRef(new Animated.ValueXY(0)).current;
  const panY = useRef(new Animated.ValueXY(0)).current;
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (e, gesture) => {
        if (gesture.dx > screenWidth ) {
            console.log("sağ")
          setOpenMenu(false);
        } else if (gesture.dx < -screenWidth ) {
          setOpenMenu(true);
        } else {
  ...
  const exitMethod = async () => {
    try {
      const exit = await AsyncStorage.removeItem("@entegrenity:jwt");
      if (exit === undefined) {
  ...
  return (
    <Animated.View
      style={[
        {
          transform: [{ translateX: panX.x }, { translateY: panY.y }],
        },
      ]}
      {...panResponder.panHandlers}
      className="p-2 w-full bg-slate-600 flex flex-col justify-between items-center relative border-b-2 border-slate-700"
    >
  ...
  c43.4,43.4,113.8,43.4,157.1,0l78.6-78.6C433.1,578.5,433.1,508.2,389.7,464.8z"
      ...
      <Pressable
        className={
          !openMenu
            ? "absolute top-4 p-2 right-[-237px] w-4/5 bg-slate-600 h-screen"
            : "absolute top-4 p-2 right-0 w-4/5 bg-slate-600 h-screen"
        }
      ...
      >
        <ScrollView onScroll={(e) => handleScroll(e)}>
          <Pressable
            className={
              openMenu
                ? "w-full flex flex-row justify-start  mt-[-10px]"
                : "w-full flex flex-row justify-start ml-[-5px] mt-[-10px]"
            }
            onPress={() => {
              setOpenMenu(!openMenu);
              setOpenProductsGroup(false);
            }}
          >
            <View>
              <Text className={openMenu ? "rotate-180" : ""}>
                {openMenu ? (
          ...
          <Pressable>
            <View
              className={
                !openMenu
                  ? "flex flex-row justify-start items-center py-0"
                  : "flex flex-row justify-start items-center py-2"
              }
            >
              <MaterialCommunityIcons name="home" size={22} color="#ea580c" />
              <Text className="text-2xl text-white font-medium pl-2">
                Anasayfa
              </Text>
            </View>
          </Pressable>
          ...
          <Pressable>
            <View
              className={
                !openMenu
                  ? "flex flex-col justify-start items-center py-0"
                  : "flex flex-col justify-start items-center py-2"
              }
            >
              <Pressable
                onPress={() => setOpenProductsGroup(!openProductsGroup)}
                className="w-full"
              >
              ...
              <View className={openProductsGroup ? "pl-2" : "hidden"}>
                <View
                  className={
                    !openMenu
                      ? "flex flex-row justify-start items-center py-0"
          ...
          <Pressable>
            <View className="pt-2 mt-5 border-t border-orange-600 ">
              <Pressable>
                <View
                  className={
                    !openMenu
                ...
                >
                  <MaterialIcons name="settings" size={22} color="#ea580c" />
                  <Text className="text-2xl text-white font-medium flex flex-row justify-start items-center pl-2">
                    Genel Ayarlar
                  </Text>
                </View>
              </Pressable>
